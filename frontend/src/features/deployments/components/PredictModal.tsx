...
import { CheckCircle2, ImageUp, Info, Loader2, Sparkles, Trash2, XCircle } from 'lucide-react'
...
export function PredictModal({
  ...
  const runPredict = async () => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      setItems((prev) => prev.map((it, idx) => (idx === i ? { ...it, pending: true, error: undefined } : it)))
      try {
        const b64 = await fileToBase64(items[i].file)
